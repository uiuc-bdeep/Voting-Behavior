...
if( on_rstudio)     # if specifying the state directly in the rscript
{
  state <- "OH"     #testing
  state_code <- 39
} else                # if specifying the state from the command line
{
  if (length(args) == 0)
  {
    print("missing argument for the state name")
    quit()
  }
  state <- commandArgs[1]
...
TD <- points_over_polygon(hedonics_output, polygon_input, points_over_polygon_output)
